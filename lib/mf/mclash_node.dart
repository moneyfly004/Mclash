...
class MclashNode {
  ...
  /// 这次测速时**内核里还没有这个节点**（订阅刚更新、内核还在用旧配置跑）。
  ///
  /// 为什么单独记：这种情况内核对 `/proxies/{name}/delay` 会返回 404，
  /// 以前一律当成「节点超时/挂了」显示「超时」——用户看到的就是
  /// 「新订阅里的节点（SSR / light* 那些）测速全部超时」，而实际上它们
  /// 只是还没进内核。界面据此改显示「未测到」，并触发一次内核重载。
  bool missingInKernel = false;
